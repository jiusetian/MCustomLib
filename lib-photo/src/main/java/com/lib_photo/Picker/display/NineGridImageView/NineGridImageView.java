...
public class NineGridImageView<T> extends ViewGroup {
    ...
    private List<ImageView> mImageViewList = new ArrayList<>(); //主要用来保存可以复用的imageview对象
    private List<T> mImgDataList; //图片数据，保存是一个泛型数据类型，里面保存的是图片信息资料
    ...
    public void setImagesData(List lists) {
        ...
        int[] gridParam = calculateGridParam(newShowCount, mShowStyle); //行数和列数
        mRowCount = gridParam[0];
        mColumnCount = gridParam[1];
        if (mImgDataList == null) { //没有设置过图片资源
            int i = 0;
            while (i < newShowCount) {
                ImageView iv = getImageView(i); //获取imageview对象
                if (iv == null) {
                    return;
                }
                addView(iv, generateDefaultLayoutParams()); //添加view到ViewGroup中
                i++;
            }
        } else { //原先设置过
            int oldShowCount = getNeedShowCount(mImgDataList.size());
            if (oldShowCount > newShowCount) { //移除多余的imageview
    ...
    private ImageView getImageView(final int position) {
        if (position < mImageViewList.size()) {
            ...
            if (mAdapter != null) {
                ...
                mImageViewList.add(imageView); //保存好，主要是为了下次复用
    ...
    protected static int[] calculateGridParam(int imagesSize, int showStyle) {
        ...
        switch (showStyle) {
            ...
            default:
            case STYLE_GRID:
                gridParam[0] = imagesSize / 3 + (imagesSize % 3 == 0 ? 0 : 1); //行数
                gridParam[1] = 3; //列数
        }
        return gridParam;
    }
