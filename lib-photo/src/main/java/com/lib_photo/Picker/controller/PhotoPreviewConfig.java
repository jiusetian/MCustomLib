...
public class PhotoPreviewConfig {
    ...
    private void startPreviewActivity(Activity activity, Bundle bundle) {
        Intent intent = new Intent(activity, PhotoPreviewEditActivity.class);
        intent.putExtra(EXTRA_BUNDLE, bundle);
        activity.startActivityForResult(intent, REQUEST_CODE); //请求码的作用，在接收act退出结果的时候，可以区分是哪个act的退出结果
