package com.lib_photo.Preview.view.image;
...
public class TransferImage extends PhotoView {
    ...
    public void transformIn() {
        cate = CATE_ANIMA_TOGETHER; //动画是一起执行
        state = STATE_TRANS_IN; //状态是进入
        transformStart = true;

        paint.setAlpha(0); //刚开始是完全透明
        invalidate();
    }
    ...
    private void initTransform() {
        ...
        //图片CENTR_CROP是按比例缩放，直到第二次有一个边刚好和imageview匹配，另外一个边一般是大于对应的边
        float xSScale = originalWidth / ((float) transDrawable.getIntrinsicWidth()); //originalWidth其实是imageview的宽度，transDrawable.getIntrinsicWidth()是图片的实际宽度
        float ySScale = originalHeight / ((float) transDrawable.getIntrinsicHeight());
        float startScale = xSScale > ySScale ? xSScale : ySScale; //取大的那个
        transform.startScale = startScale;
        /* 计算结束时候的缩放值，结束值因为要达到FIT_CENTER效果，所以要保证图片的宽和高至少1个能匹配原始的宽和高，另1个小于 */
        //FIT_CENTER是按比例缩放，直到第一次有一个边刚好和imageview匹配，另外一个边一般是小于对应的边
        ...
        /**
         * 计算Canvas Clip的范围，也就是图片的显示的范围，因为图片是慢慢变大，并且是等比例的，
         * 所以这个效果还需要裁减图片显示的区域
        ...
        transform.startRect.left = originalLocationX; //矩形左坐标
        transform.startRect.top = originalLocationY; //矩形top坐标
        transform.startRect.width = originalWidth; //矩形的宽度
        transform.startRect.height = originalHeight; //矩形高度
        /* 结束区域 */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (state != STATE_TRANS_NORMAL) { //非普通状态
            ...
            canvas.drawPaint(paint); //设置画笔

            int saveCount = canvas.getSaveCount();
            canvas.save();
            // 先得到图片在此刻的图像Matrix矩阵
            calcBmpMatrix();
            canvas.translate(transform.rect.left, transform.rect.top); //rect用来保存区域信息
            canvas.clipRect(0, 0, transform.rect.width, transform.rect.height); //显示区域
            //画布关联矩阵，画出来的内容按矩阵改变，而不是画布改变
            canvas.concat(transMatrix);
            getDrawable().draw(canvas); //将Drawable画到Canvas中
        ...
        if (stage == STAGE_TRANSLATE) { // 平移动画
            ...
            valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public synchronized void onAnimationUpdate(ValueAnimator animation) {
                    paint.setAlpha((int) (255 * animation.getAnimatedFraction())); //透明度随动画的改变而改变
