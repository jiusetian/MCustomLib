...
class TransferLayout extends FrameLayout {
            ...
            if (cate == TransferImage.CATE_ANIMA_TOGETHER) {
                switch (state) {
                    case TransferImage.STATE_TRANS_IN: // 伸展动画执行完毕
                        addIndexIndicator(); //添加页面指示
                        addExView(); //添加小图标
                        transViewPager.setVisibility(View.VISIBLE);
                        removeFromParent(transImage); //移除动画的view
                        break;
                    case TransferImage.STATE_TRANS_OUT: // 退出动画
                        resetTransfer();
                        break;
                }
            } else { // 如果动画是分离的
    ...
    private void loadSourceImageOffset(int position, int offset) {
        ...
        if (!loadedIndexSet.contains(position)) { //没有加载过
    ...
    private void createTransferViewPager() {
        ...
        transViewPager.setIAnimClose(new DragViewPager.IAnimClose() {
            ...
            @Override
            public void onPictureRelease(View view, float detalX, float detalY, float scale, float alpha) {
                dismiss(transConfig.getNowThumbnailIndex(), view, detalX, detalY, scale, alpha);
            }
        });
    ...
    public void show() {
        ...
        int nowThumbnailIndex = transConfig.getNowThumbnailIndex(); //当前显示的图片
    ...
    TransferState getTransferState(int position) {
        ...
        if (!transConfig.isThumbnailEmpty()) { // 客户端指定了缩略图路径集合
            transferState = new RemoteThumState(this); //远程
        } else {
            String url = transConfig.getSourceImageList().get(position); //获取对应显示图片的URL

            if (transConfig.getImageLoader().isLoaded(url)) { //是否有缓存
    ...
    public void dismiss(int pos, View view, float detalX, float detalY, float scale, float alpha) {
        if (transImage != null && transImage.getState() == TransferImage.STATE_TRANS_OUT) // 防止双击
            return;
        transImage = getTransferState(pos).transferOut(pos);
        //设置移动过的view信息
        //==null代表当前退出的图片打开之前是没有在界面有显示的
        if (transImage == null)
            diffusionTransfer(pos, scale);
        else
            transViewPager.setVisibility(View.INVISIBLE); //viewpager进行隐藏
        transImage.setChangeInfo(detalX, detalY, scale, alpha);
        hideIndexIndicator();
    }
     ...
     */
    private void diffusionTransfer(int pos, float startScale) {
        ...
        PropertyValuesHolder scaleXHolder = PropertyValuesHolder.ofFloat("scaleX", 1 * startScale, 1.2f * startScale);
     ...
     * 在面板中添加额外小图标
     */
    private void addExView() {
        ExView exView = transConfig.getExView();
    ...
    private void addIndexIndicator() {
        ...
        if (indexIndicator != null && transConfig.getSourceImageList().size() >= 2) {
            indexIndicator.attach(this); //添加
            indexIndicator.onShow(transViewPager); //显示
        }
    }
