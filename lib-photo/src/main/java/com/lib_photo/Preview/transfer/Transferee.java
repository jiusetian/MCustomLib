...
import com.lib_photo.Preview.loader.UniversalImageLoader;
...
public class Transferee implements DialogInterface.OnShowListener,
    ...
    private void fillByRecyclerView(final List<ImageView> originImageList) {
        ...
        int firstPos = 0, lastPos = 0; //第一个可见和最后一个可见item的位置
    ...
    public Transferee apply(TransferConfig config) {
        if (!shown) {
            transConfig = config;
            fillOriginImages(); //获取原来显示缩略图的imageview集合
            checkConfig(); //检查配置
            transLayout.apply(config); //应用
        }
        return this;
    }
