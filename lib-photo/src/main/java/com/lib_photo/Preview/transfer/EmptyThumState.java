...
class EmptyThumState extends TransferState {
    ...
    public void prepareTransfer(final TransferImage transImage, final int position) {
        //如果不指定缩略图或者没有图片缓存，此时viewpager中的imageview提前准备的是原来缩略图的显示
    ...
    public void transferLoad(final int position) {
        ...
        Drawable placeHolder; //加载完成之前显示的占位图
    ...
    private Drawable getPlacHolder(int position) {
        ...
        if (originImage != null) {
            placeHolder = originImage.getDrawable(); //原来的缩略图
        } else {
     ...
     *
     * @param targetImage 被裁剪的 TransferImage，为viewpager中的imageview
     * @param position    图片索引
    ...
    private Drawable clipAndGetPlachHolder(TransferImage targetImage, int position) {
        ...
        Drawable placeHolder = getPlacHolder(position); //获取原来缩略图或者站位图
        int[] clipSize = new int[2]; //原来缩略图的imageview的大小
        ImageView originImage = config.getOriginImageList().get(position);
        if (originImage != null) {
