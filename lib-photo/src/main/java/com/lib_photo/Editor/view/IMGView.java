package com.lib_photo.Editor.view;
...
import com.lib_photo.Editor.core.IMGImage;
import com.lib_photo.Editor.core.IMGMode;
import com.lib_photo.Editor.core.IMGPath;
import com.lib_photo.Editor.core.IMGText;
import com.lib_photo.Editor.core.anim.IMGHomingAnimator;
import com.lib_photo.Editor.core.homing.IMGHoming;
import com.lib_photo.Editor.core.sticker.IMGSticker;
import com.lib_photo.Editor.core.sticker.IMGStickerPortrait;
...
public class IMGView extends FrameLayout implements Runnable, ScaleGestureDetector.OnScaleGestureListener,
    ...
    private void startHoming(IMGHoming sHoming, IMGHoming eHoming) {
        if (mHomingAnimator == null) {
            ...
            mHomingAnimator.addUpdateListener(this); //更新进度相关监听
            mHomingAnimator.addListener(this); //动画的监听
        }
    ...
    private void onDrawImages(Canvas canvas) {
        ...
        // clip 中心旋转
        RectF clipFrame = mImage.getClipFrame(); //剪裁图片边框
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed) { //view当前是match_parent
            Log.d(TAG, "onLayout: 屏幕宽高="+ IMGUtils.getScreenWidth(getContext())+"/////"+IMGUtils.getScreenHeight(getContext()));
    ...
    public void onAnimationUpdate(ValueAnimator animation) {
        ...
        toApplyHoming((IMGHoming) animation.getAnimatedValue()); //因为自定义了插值器，所有这里获取AnimatorValue的时候，就会返回自己定义的那个插值器对象
    }
    ...
    private boolean onScroll(float dx, float dy) {
        IMGHoming homing = mImage.onScroll(getScrollX(), getScrollY(), -dx, -dy); //不是剪裁模式时，返回null
        if (homing != null) {
            toApplyHoming(homing);
            return true;
        }
        //第一次的时候，还没调用scrollTo方法，只是监听到了手势的滑动距离，此时要调用scrollTo方法，将图片进行移动
        Log.d(TAG, "onScroll: 移动的距离="+getScrollY());
        return onScrollTo(getScrollX() + Math.round(dx), getScrollY() + Math.round(dy)); //进行滚动
    }
    ...
    private class MoveAdapter extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            //distanceX是X轴方向的距离，整个距离是上次调用onScroll和当前调用onScroll手势在X轴方向的滑动距离，而不是e1和e2的之间的距离,有点代表是移动速度
