package com.lib_photo.Editor.core;
...
import com.lib_photo.Editor.core.clip.IMGClip;
import com.lib_photo.Editor.core.clip.IMGClipWindow;
...
//                    mClipWin
            ...
            win.offset(scrollX, scrollY); //相当于在原来坐标上加上去
            //移动图片的时候，显示边框是不会改变的，但是win会改变
    ...
    public void onWindowChanged(float width, float height) {
        ...
        mWindow.set(0, 0, width, height); //当view是match_parent的时候，相当于整个屏幕
        if (!isInitialHoming) {
    ...
    private void onInitialHoming(float width, float height) {
        mFrame.set(0, 0, mImage.getWidth(), mImage.getHeight()); //大小为图片的大小
        mClipFrame.set(mFrame); //可剪裁区域也是图片大小
        mClipWin.setClipWinSize(width, height); //计算剪裁窗口区域
    ...
    public void onDrawImage(Canvas canvas) {
        ...
        if (DEBUG) {
            ...
            canvas.drawRect(mWindow,mPaint);
        }
    }
